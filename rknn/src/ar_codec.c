...
static DcvcRkStatus run_sp_4x(DcvcRkArCodec* c, const float* yhat, int H, int W, int round)
{
    ...
    return run1(c->eng_spatial_prior, ws->sp_out, 2 * nc, H, W, ws->cat, 2 * nc, H, W);
}
                              ...
                              int H, int W)
{
    ...
    return run1(c->eng_spatial_prior, ws->cat, 4 * nc, H, W, ws->sp_out, 2 * nc, H, W);
}
                              ...
                              float* y_hat_out)
{
    ...
    for (int round = 0; round < 4; round++) {
        ...
        if (round > 0) {
            st = run_sp_4x(c, ws->yhat, H, W, round);
            if (st != DCVC_RK_OK) return st;
    ...
    return dcvc_rans_encoder_get_stream(c->rans_enc, out_stream, out_size) == 0
                              ...
                              float* y_hat_out)
{
    ...
    st = run_sp_2x(c, ws->yhat_step, params, H, W);
    if (st != DCVC_RK_OK) return st;
