...
//
// Closed-loop self-consistency test (Phase 1 + 2 of the closed-loop plan).
//
// Both the encoder and decoder use the SAME TensorRT plugin engines (cuBLAS
// DepthConv), so their FP16 arithmetic is bit-identical.  This test proves that
// encode → rANS bitstream → decode reproduces y_hat exactly, with no dependence
// on PyTorch golden data.
//
// Pipeline:
//   ENCODE: y*q_enc → 4x AR (process_mask_yq → build_index_enc → rANS encode_y
//           → restore_y_4x) → bitstream + y_hat_enc
//   DECODE: bitstream → 4x AR (build_index_dec → rANS decode_y → restore_y_4x)
//           → y_hat_dec
//   COMPARE: y_hat_enc == y_hat_dec  (must be 100 % identical)
...
/* ---- FP16 helpers (identical to test_y_decode_4x) ---- */
...
int main(int argc, char** argv) {
    ...
    printf("Kernels: build_dec=%p build_enc=%p restore=%p sp4x=%p mul=%p pmask_yq=%p\n",
           p_build_idx_dec,p_build_idx_enc,p_restore,p_sp4x,p_mul,p_pmask_yq);
    ...
    int HW=pf_H*pf_W;
    printf("params_fusion: [%d,%d,%d,%d]  N_ch=%d HW=%d\n",pf_npy.dims[0],pf_C,pf_H,pf_W,N_CH,HW);
    ...
    /* ---- separate_prior: q_enc(ch0), q_dec(ch1), scales(2:258), means(258:514) ---- */
    void* d_qenc; cudaMalloc(&d_qenc, HW*2);
    void* d_qdec; cudaMalloc(&d_qdec, HW*2);
    uint16_t *qenc_h=malloc(HW*2), *qdec_h=malloc(HW*2);
    for(int i=0;i<HW;i++){
        float qe=1.0f/(1.0f+expf(-h2f(pf_host[0*HW+i])))*1.5f+0.5f;  /* ch 0 */
        float qd=1.0f/(1.0f+expf(-h2f(pf_host[1*HW+i])))*1.5f+0.5f;  /* ch 1 */
    ...
    /* ---- y_spatial_prior_reduction: Conv2d(514→256) on full params → common_params ---- */
    DcvcRtStatus st;
    ...
    printf("yspr → [%d,%d,%d,%d] (common_params)\n",rd[0],rd[1],rd[2],rd[3]);

    /* ---- Load 3 adaptor engines + y_spatial_prior engine ---- */
    ...
    printf("Loaded 3 adaptors + ysp + yspr\n");
    ...
    /* ---- Synthesize a deterministic y latent [1,256,16,16] ---- */
    ...
    /* ---- y_scaled = y * q_enc (broadcast q_enc across channels) ---- */
    ...
    /* ---- Shared workspace ---- */
    int R=N_CH/4;  /* reduced channel count = 64 */
    ...
    /* saved per-round host data for cross-checking encode vs decode */
    /* R*HW = 64*256 = 16384 elements per round */
    ...
    for(int round=0; round<4; round++){
        ...
        p_mul(curr_scales, d_masks[round], d_smask, N_CH*HW, 0);
        p_sp4x(d_smask, d_sr, R*HW, 0);
        cudaMemcpy(dec_sr[round], d_sr, R*HW*2, cudaMemcpyDeviceToHost);        /* build_index_dec(scales_r) → indexes */
        p_build_idx_dec(d_sr, (uint8_t*)d_indexes, scale_min, scale_max,
                        log_scale_min, log_step_recip, R*HW, 0);
        cudaMemcpy(indexes_h, d_indexes, R*HW, cudaMemcpyDeviceToHost);        /* rANS decode_y */        dcvc_rans_decoder_decode_y(dec, indexes_h, R*HW, g_cdf_idx);        int8_t* syms=NULL; size_t sym_n=0;
        dcvc_rans_decoder_get_symbols(dec, (int8_t**)&syms, &sym_n);        memcpy(dec_sym[round], syms, sym_n<R*HW?sym_n:R*HW);
        /* compare symbols: enc packed (sym<<8|idx) vs dec symbols */
        {
        ...
        /* compare scales_r: enc vs dec */
        {
            int smatch=0; float smax=0;
            for(int i=0;i<R*HW;i++){float d=fabsf(h2f(enc_sr[round][i])-h2f(dec_sr[round][i]));if(d>smax)smax=d;if(d<0.001f)smatch++;}
            printf("  dec round %d: scales_r enc-vs-dec match=%d/%d max_abs=%.6f\n",round,smatch,R*HW,smax);
        }
        /* y_q_r as fp16, restore, accumulate */
        {
