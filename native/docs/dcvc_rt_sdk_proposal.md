...
>
> **更新说明**：本文档已根据 `native/` 目录的实际落地进展更新。初始方案曾评估 ONNX Runtime + TensorRT EP，实际实现采用更贴近硬件的 TensorRT C API + 自研 CUDA Plugin/Kernel 方案。
...
完整映射见 [native/docs/ops_inventory.md](native/docs/ops_inventory.md)。
...
完整头文件见 [native/include/dcvc_rt.h](native/include/dcvc_rt.h)。
...
- 输出 [native/docs/ops_inventory.md](native/docs/ops_inventory.md)，明确每个算子去向（TRT / Plugin / Host / CPU）。
- 完成 CMake 构建与测试框架。
