...
static DcvcRtStatus ps_ensure(int H, int W)
{
    ...
    if (!g_state.kernel_so) {
        ...
        g_state.kernel_so = dlopen("tensorRT/build/plugin_demo/libdcvc_kernels.so", RTLD_NOW | RTLD_GLOBAL);
        if (!g_state.kernel_so) {
                                ...
                                DcvcRansDecoder* rans_dec)
{
    ...
    char path[640];
    const char* ad = "tensorRT/assets"; /* will be overridden by runner's asset_dir */
