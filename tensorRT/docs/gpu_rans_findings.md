...
DCVC-RT native 运行时，纯 C + TensorRT + CUDA kernel，无 pip 依赖。y 与 z 均用 CPU rANS（`tensorRT/rans/rans.cpp`，`RansEncoder/DecoderLibMultiThread`，**编/解码各有一条 `std::thread`**——这一点对 §6 的判定是决定性的）。
...
- `tensorRT/rans/rans.h:128,192` + `rans.cpp:264,444`：`RansEncoder/DecoderLibMultiThread` 各有一条 `std::thread`，`encode_z/decode_z` 投递任务后**立即返回**，结果在后续取。
...
cmake -S tensorRT -B tensorRT/build -DDCVC_RT_HAS_TENSORRT=ON -DDCVC_RT_HAS_CUDA=ON -DDCVC_RT_BUILD_TESTS=ON
cmake --build tensorRT/build -j8
CUDA_VISIBLE_DEVICES=0 ./tensorRT/build/test_quality 32          # 质量+BPP
CUDA_VISIBLE_DEVICES=0 ./tensorRT/build/test_perf 32 8           # 性能
cd tensorRT && ctest --test-dir build --output-on-failure         # 全量 15 项
```
...
cd /root/workspace/DCVC && cd tensorRT && cmake --build build -j8
CUDA_VISIBLE_DEVICES=0 ./tensorRT/build/test_perf 32 100    # e2e ~20.2ms
```
...
CUDA_VISIBLE_DEVICES=0 ./tensorRT/build/test_perf 32 100   # 原生: 20.3ms e2e
```
