...
产出 `dcvc_rt` C API（encoder/decoder），用于高性能部署。代码在 `tensorRT/`。

## 二、已完成的工作

### 工程脚手架（完整可用）
- `tensorRT/CMakeLists.txt`：CMake 构建，开关 `DCVC_RT_HAS_TENSORRT` / `DCVC_RT_BUILD_TESTS`
- `tensorRT/include/dcvc_rt.h`：公开 C API（config/encoder/decoder/frame/packet）
- `tensorRT/src/`：`dcvc_rt.c`(420行 orchestrator)、`bitstream.c`、`color.c`、`dpb.c`、
  `trt_engine.cpp`(TRT 引擎封装)、`trt_runner.c`
- `tensorRT/rans/`：rANS 熵编解码 C++ 移植（`rans.cpp`/`rans_byte.h`/`rans_c.cpp` + C wrapper `rans_c.h`）
- `tensorRT/plugins/`：5 个融合算子 plugin（DepthConv/SubpelConv2x/ProcessWithMask/BiasShuffle/kernels）
...
- `test_y_decode_4x`：4 轮 AR 空间先验解码，能成功加载 3 个 adaptor engine + y_spatial_prior engine，
  完整跑通 decode 链路。**注意**：默认 `plugin_dir` 是相对路径 `"tensorRT/build/plugin_demo"`，
  必须从**仓库根目录**执行：`tensorRT/build/test_y_decode_4x`
...
### op 归类文档
- `tensorRT/docs/ops_inventory.md`：intra/inter 全部子图 → TRT/Plugin/Host 映射，CUDA op → plugin 对照表（P0/P1）

### 编码侧雏形（已搭但未接线）
- `dcvc_rans_encoder_*` API 已在 `rans_c.h` 定义（encode_y / encode_z / flush / get_stream）
- `tensorRT/plugins/include/dcvc_ar_prior.h` + `plugins/src/ar_prior.c`：AR 先验 host 循环
  （`dcvc_ar_prior_encode_pass`），镜像 PyTorch 的 `compress_prior_4x`，但 TRT 绑定还是空的

## 三、核心问题：FP16 精度不 bit-exact

详见 `tensorRT/docs/trt_precision_findings.md`（状态标注「已暂停」）。
...
`tensorRT/tests/test_closed_loop.c`（432 行）实现了完整的 encode + decode 4× AR 循环：
...
`test_closed_loop` 实测结果（从仓库根执行 `tensorRT/build/test_closed_loop`）：
```
...
**ctest 全量结果**（从 `tensorRT/build` 执行 `ctest`）：
```
...
| 文件                                    | 作用                                                              |
| --------------------------------------- | ----------------------------------------------------------------- |
| `tensorRT/tests/test_closed_loop.c`       | **闭环验证（阶段 1+2 产物）**：encode→bitstream→decode，100% 匹配 |
| `tensorRT/docs/ops_inventory.md`          | op → TRT/Plugin 映射（本次会话的 active file）                    |
| `tensorRT/docs/trt_precision_findings.md` | 精度问题根因 + 三方案分析                                         |
| `tensorRT/tests/test_y_decode_4x.c`       | **decode 4× AR 循环参考实现**（401行，核心）                      |
| `tensorRT/tools/build_plugin_engines.py`  | TRT engine 构建（walk PyTorch model → plugin）                    |
| `tensorRT/rans/rans_c.h` / `rans_c.cpp`   | rANS C API（encoder/decoder 都有）                                |
| `tensorRT/plugins/src/dcvc_kernels.cu`    | CUDA kernels（含新增 `dcvc_k_process_mask_yq`）                   |
| `tensorRT/plugins/src/ar_prior.c`         | encode AR 循环雏形（TRT binding 待填）                            |
| `tensorRT/src/dcvc_rt.c`                  | C API orchestrator（**已接线 intra+inter，I→P→P 跑通**）                     |
...
- 构建：`cmake -S native -B tensorRT/build -DDCVC_RT_HAS_TENSORRT=ON -DDCVC_RT_BUILD_TESTS=ON`
- 跑 GPU 测试须从仓库根：`tensorRT/build/test_y_decode_4x`
...
**1. 2× AR codec 修复与验证（`tensorRT/src/dcvc_ar_codec.c`）**
...
**2. Inter 全流水线（`tensorRT/src/dcvc_inter_pipeline.c` + `.h`，新文件）**
...
### 新增/修改文件
- 新增：`tensorRT/src/dcvc_inter_pipeline.c`、`tensorRT/include/dcvc_inter_pipeline.h`
- 新增测试：`test_inter_ar`、`test_inter_pipeline`、`test_sdk_inter`（均注册 ctest）
- 改：`tensorRT/src/dcvc_ar_codec.c`（2× 修复）、`tensorRT/src/dcvc_rt.c`（inter 接线）、`tensorRT/CMakeLists.txt`

### 构建/测试
```
cd /root/workspace/DCVC && cd tensorRT/build
...
- 重建内核 `.so`：`/usr/local/cuda-13.2/bin/nvcc -shared -Xcompiler -fPIC -O2 -arch=sm_80 -o tensorRT/build/plugin_demo/libdcvc_kernels.so tensorRT/plugins/src/dcvc_kernels.cu`
