...
//
// End-to-end heterogeneous stability test: load y from CPU ONNX output and run
// the native 4x AR encode/decode loop.  Verifies that the small CPU ONNX error
// does not make rANS crash.
//
// Usage: test_ar_with_cpu_y <asset_dir> <plugin_dir> <y_npy>
...
int main(int argc, char** argv){
    ...
    const char* y_npy = argc>3 ? argv[3] : "../../tensorRT/assets/golden/intra_analysis_output.npy";
    ...
    /* Load y from CPU ONNX output .npy */
    Npy y_npy_obj;
    if(read_npy(y_npy,&y_npy_obj)!=0){fprintf(stderr,"failed to read y %s\n",y_npy);return 1;}
    if(y_npy_obj.dims[0]!=1||y_npy_obj.dims[1]!=N_CH||y_npy_obj.dims[2]!=pf_H||y_npy_obj.dims[3]!=pf_W){
        fprintf(stderr,"bad y shape [%d,%d,%d,%d]\n",y_npy_obj.dims[0],y_npy_obj.dims[1],y_npy_obj.dims[2],y_npy_obj.dims[3]);
        return 1;
    }
    void* d_y; cudaMalloc(&d_y, N_CH*HW*2);
    npy_to_dev_fp16(&y_npy_obj, d_y);
    printf("Loaded y from %s: shape [%d,%d,%d,%d]\n", y_npy, y_npy_obj.dims[0], y_npy_obj.dims[1], y_npy_obj.dims[2], y_npy_obj.dims[3]);
    ...
    /* COMPARE */
    {
        ...
        if(exact==n && all_sym_match){
            printf("\n*** CPU-ONNX y END-TO-END: PASS (rANS stable) ***\n");
        } else {
            printf("\n*** CPU-ONNX y END-TO-END: FAIL ***\n");
        }
        free(eh);free(dh);
    }
    ...
    free(y_npy_obj.fp16); free(y_npy_obj.fp32); free(y_npy_obj.i32);
    return 0;
}
