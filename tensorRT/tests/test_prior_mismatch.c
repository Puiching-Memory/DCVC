...
//
// Tolerance sweep: measure how much prior (params_fusion) error the 4x AR rANS
// decoder can tolerate before symbol mismatch appears.  Prints a JSON-like summary
// line per run, which can be aggregated by a driver script.
//
// Usage: test_prior_mismatch <asset_dir> <plugin_dir> [noise_mag] [seed]
...
static int write_fp16_npy(const char* path, const float* data, const int* dims, int nd)
{
    FILE* f = fopen(path, "wb");
    if (!f) return -1;
    fwrite("\x93NUMPY", 1, 6, f);
    uint8_t major = 1, minor = 0;
    fwrite(&major, 1, 1, f); fwrite(&minor, 1, 1, f);
    char shape[256] = "";
    int pos = 0; pos += snprintf(shape + pos, sizeof(shape) - pos, "(");
    for (int i = 0; i < nd; i++) pos += snprintf(shape + pos, sizeof(shape) - pos, "%d,", dims[i]);
    pos += snprintf(shape + pos, sizeof(shape) - pos, ")");
    char hdr[512];
    int len = snprintf(hdr, sizeof(hdr), "{'descr': '<f2', 'fortran_order': False, 'shape': %s, }", shape);
    ...
    for (size_t i = 0; i < n; i++) h[i] = f32h(data[i]);
    fwrite(h, 2, n, f);
    free(h); fclose(f);
    return 0;
}
...
int main(int argc, char** argv){
    ...
    float noise_mag = argc>3 ? (float)atof(argv[3]) : 0.01f;
    ...
    for(int round=0; round<4; round++){
        ...
        memcpy(enc_sym[round], packed_h, R*HW*2);
    ...
    /* ====================== PERTURB DECODER PRIOR ====================== */
    {
        uint16_t* pf_dec = (uint16_t*)malloc(pf_npy.elems*2);
        cudaMemcpy(pf_dec, d_pf, pf_npy.elems*2, cudaMemcpyDeviceToHost);
        float maxv=0, sumv=0; size_t n=pf_npy.elems;
        for(size_t i=0;i<n;i++){
            float v=h2f(pf_dec[i]);
            float noise = v * ((float)rand()/RAND_MAX - 0.5f) * 2.0f * noise_mag;
            pf_dec[i] = f32h(v + noise);
            float ad=fabsf(noise); sumv+=ad; if(ad>maxv) maxv=ad;
        }
        cudaMemcpy(d_pf, pf_dec, pf_npy.elems*2, cudaMemcpyHostToDevice);
        free(pf_dec);
        /* Re-run reduction with perturbed prior so d_common matches the decoder's view */
        dcvc_trt_engine_execute(eng_yspr,NULL); cudaDeviceSynchronize();
        printf("Perturbed decoder prior: noise_mag=%.4f actual avg_abs=%.6f max_abs=%.6f\n", noise_mag, sumv/n, maxv);
    }

    /* DECODE with perturbed prior */
    ...
    int all_sym_match=1;
    float total_match_pct=0;
    float min_match_pct=100.0f;
    for(int round=0; round<4; round++){
        void *curr_scales=d_scales0, *curr_means=d_means0;
        if(round>0){
        ...
        dcvc_rans_decoder_get_symbols(dec, (int8_t**)&syms, &sym_n);        memcpy(dec_sym[round], syms, sym_n<R*HW?sym_n:R*HW);
        {
            ...
            float pct=100.0f*match/(R*HW);
            total_match_pct += pct;
            if(pct<min_match_pct) min_match_pct=pct;
            printf("  dec round %d: sym match=%d/%d (%.1f%%)",round,match,R*HW,pct);
...

    {
        ...
        if(exact==n && all_sym_match){
            printf("\n*** PRIOR MISMATCH: UNEXPECTED PASS ***\n");
        } else {
            printf("\n*** PRIOR MISMATCH: FAIL AS EXPECTED (prior mismatch breaks rANS) ***\n");
        }
        printf("SUMMARY noise_mag=%.6e seed=%d all_match=%d min_match_pct=%.3f avg_match_pct=%.3f exact_y=%d/%d max_y_err=%.6e\n",
               noise_mag, seed, all_sym_match, min_match_pct, total_match_pct/4.0f, exact, n, mx);

        /* Save encoder/decoder y_hat as fp16 .npy for visual inspection */
        int dims[4] = {1, N_CH, pf_H, pf_W};
        write_fp16_npy("yhat_enc.npy", (float*)eh, dims, 4);
        write_fp16_npy("yhat_dec.npy", (float*)dh, dims, 4);
        printf("saved yhat_enc.npy and yhat_dec.npy\n");

        free(eh);free(dh);
    }

    free(stream); free(packed_h); free(indexes_h); cudaFree(d_indexes); free(pf_host); free(qenc_h); free(qdec_h);
    free(enc_sym); free(dec_sym);
